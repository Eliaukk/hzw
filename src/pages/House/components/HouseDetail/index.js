...
import { getHouseDetailReq, getCollectStatusReq, addCollectReq, delCollectReq } from '../../../../api/house'
...
export default class HouseDetail extends Component {
  ...
  async componentDidMount() {
    ...
    if (flag) {
      ...
      // 获取文章收藏状态
      const res = await getCollectStatusReq(id)
      if (res.data) {

        const { data: { isFavorite } } = res
        // 更新是否收藏
        this.setState({
          isFavorite
        })
        ...
        this.props.history.replace({ pathname: '/login', backURL: path })
      }
  ...
  async getHouseDetail() {
    ...
    // 发送请求获取房屋详情数据
    const { status, data } = await getHouseDetailReq(id)
    // 更新房屋数据
    this.setState({
      houseInfo: data
    })
  ...
  hClick = async () => {
    ...
    if (!this.isLogin()) {
      // 前往登录页面
      return Modal.alert('温馨提示', '您还未登录，是否前往登录页面？', [{ text: '取消' }, {
        text: '确定', onPress: () => {
          this.props.history.replace({ pathname: '/login', backURL: CurURL })
        }
      }])
    }
    // 获取当前收藏状态
    ...
    if (status) {
      // 收藏就移除搜藏
      await delCollectReq(ID)

    } else {
      await addCollectReq(ID)
        ...
        <Flex className={styles.fixedBottom}>
          <Flex.Item onClick={this.handleFavorite}>
            <img
              ...
              alt="收藏"
            />
            <span className={styles.favorite} onClick={this.hClick}>
              {isFavorite ? '已收藏' : '收藏'}
            </span>
          </Flex.Item>
